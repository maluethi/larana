...
filename = "/home/data/uboone/laser/7267/out/roi/LaserReco-LaserHit-7267-0875_digitfilter-exp-roi.root"
...
for evt in range(50):
    ...
    for idx in range(len(track_z)):
        z, x, width = track_z[idx], track_x[idx], width_x[idx]
        dz = z - last_z
    ...
    plt.plot(track_z, track_x, "o", alpha=0.2)
    plt.errorbar(ts, xs, yerr=np.array(ps))
    plt.plot(ts, ps)

    plt.plot(ts, pr, '.')

    plt.plot([laser_entry[2], laser_exit[2]], [laser_entry[0], laser_exit[0]], 'r-')
    plt.show()
